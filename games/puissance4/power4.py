...
def init(grid):
    ...
    if "init" not in content:
        print_error("BAD_FORMAT", message="init key is missing", fatal=True)
    init_content = content["init"]
    if "players" not in init_content:
        print_error("MISSING_ARGUMENT", arg=PLAYER_ARG, fatal=True)
    if init_content["players"] != 2:
        print_error(
            "INCORRECT_VALUE", arg=PLAYER_ARG, value=init_content["players"], fatal=True
        )
...
def turn(grid):
    ...
    if "actions" not in content:
        print_error("BAD_FORMAT", message="actions key is missing")
        return False
    actions = content["actions"]
    if not isinstance(actions, list):
        print_error("BAD_FORMAT", message="actions value is not a list")
        return False
    if len(actions) != 1:
        print_error("BAD_FORMAT", message="exactly one action is expected")
        return False
    action = actions[0]
    if not isinstance(action, dict):
        print_error("BAD_FORMAT", message="action is not a dict containing player, x and y values")
...
class Grid:
    def __init__(self, case_size=100):
        ...
        self.__grid = [[0] * 7 for _ in range(6)]
    ...
    def click(self, x, y):
        ...
        self.__grid[y][x] = self.__current_player
    ...
    def current_display(self):
        ...
        for x in range(7):
            for y in range(6):
                if self.__grid[y][x] == 0:
                    continue
                color = "blue" if self.__grid[y][x] == 1 else "red"
                content.append(
                    {
                        "tag": "circle",
    ...
    def current_instructions(self):
        ...
        return {
            ...
            "game_state": {"scores": scores, "game_over": game_over},
        }
